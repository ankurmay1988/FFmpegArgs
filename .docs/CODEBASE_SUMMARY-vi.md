...
| | `FFmpegArgs.Outputs.Muxers` | **Typed muxer**: [BaseMuxer](../FFmpegArgs.Outputs.Muxers/BaseMuxer.cs) → [BaseMp4Muxer](../FFmpegArgs.Outputs.Muxers/ImageMuxers/BaseMp4Muxer.cs) → [MovMuxer](../FFmpegArgs.Outputs.Muxers/ImageMuxers/MovMuxer.cs) (`-f mov`) / [Mp4Muxer](../FFmpegArgs.Outputs.Muxers/ImageMuxers/Mp4Muxer.cs) (`-f mp4`) chia sẻ `-movflags`; `MuxerExtensions` (`MovMux`/`Mp4Mux`). |
...
| 5 | ~~`Inputs.Demuxers` / `Outputs.Muxers` trùng giữa typed & extension generic + bug namespace `BaseMuxer`~~ | **Đã hợp nhất** về typed API (`input.XxxDemux`/`output.XxxMux`): thêm `Image2Demuxer`, `MovMuxer`/`Mp4Muxer`; dời `-re` sang generic; xóa `MuxerDemuxerOptionsExtension`; sửa namespace `BaseMuxer` | [Image2Demuxer.cs](../FFmpegArgs.Inputs.Demuxers/ImageDemuxers/Image2Demuxer.cs), [BaseMp4Muxer.cs](../FFmpegArgs.Outputs.Muxers/ImageMuxers/BaseMp4Muxer.cs) |
...
- **Muxer/demuxer options** — **đã hợp nhất về typed API** (`input.XxxDemux(d => …)` / `output.XxxMux(m => …)`):
  - Demuxer typed [FFmpegArgs.Inputs.Demuxers](../FFmpegArgs.Inputs.Demuxers/): 4 ImageDemuxer (Apng/Asf/Dash/Rawvideo) + `Image2Demuxer` (`-start_number`/`-pattern_type`, ép `-f image2`); `DemuxerExtensions` hậu tố `*Demux` (overload bare + `Action<TDemuxer>`).
  - Muxer typed [FFmpegArgs.Outputs.Muxers](../FFmpegArgs.Outputs.Muxers/): `MovMuxer`(`-f mov`)/`Mp4Muxer`(`-f mp4`) chia sẻ `-movflags` (enum `MovFlag`) qua `BaseMp4Muxer`; `MuxerExtensions` (`MovMux`/`Mp4Mux`).
  - `-re` giữ generic ở [InputOutputOptionsExtension.cs](../FFmpegArgs.Extensions/InputOutputOptionsExtension.cs); interface `IDemux`/`IMux`/`IAudioDemux`/`IImageDemux`/`IAudioMux`/`IImageMux`; `Format(DemuxingFileFormat/MuxingFileFormat)` ràng buộc `IDemux`/`IMux`; thêm overload `SetOption(Size)` trong [BaseOption.cs](../FFmpegArgs.Cores/BaseOption.cs). `MuxerDemuxerOptionsExtension` đã xóa.
